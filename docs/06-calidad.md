...
|---|---|---|---|
| Unitario | Vitest | Las reglas de dominio (`Plan`, `Store`, `selectors`) y la lógica de sincronización (`sync`), de materias compartidas (`espejo`), el layout del árbol (`arbolLayout`) y la medición de retención (`analytics`) | 128 tests |
...
En total, **179 tests automatizados** que corren en cada push. Ninguna versión se publica si alguno falla.

## 6.2 Tests unitarios (128)
...
flowchart LR
    ...
    L --> U["unit + integridad · vitest · 166"]
