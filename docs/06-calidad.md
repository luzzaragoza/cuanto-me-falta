...
|---|---|---|---|
| Unitario | Vitest | Las reglas de dominio (`Plan`, `Store`, `DB`, `Avance`), el editor de planes (`Borrador`, `Diff`, `Pasos`, `Historial`), la **validación** (`Validacion`), la sincronización (`RemoteData`), el layout del árbol (`Grafo`/`Layout`), el **registro de datos académicos** (`Registro`), los **permisos** (`SesionAdmin`) y el **repositorio** contra Supabase (`RepositorioPlanes`) | 294 tests |
...
En total, **319 tests automatizados** que corren en cada push. Ninguna versión se publica si alguno falla.

> El comando que chequea los tipos es **`tsc -b`**, no `tsc --noEmit`: el `tsconfig.json` raíz es *solution-style* (`"files": []` + referencias), así que `--noEmit` sale en cero **sin mirar una línea**. Es el que corre `npm run build`.

## 6.2 Tests unitarios (294)
...
- **`SesionAdmin`, `PlanAdmin`, `PlanNuevo` y `UniversidadNueva` (44):** las reglas de acceso a la administración de planes (ADR-12, ADR-14): quién entra a `#admin` (un `admin_uni` **sin** habilitaciones no entra: el rol solo no alcanza), qué puede en cada universidad —que es **una sola pregunta**, no un permiso por acción, y el superadmin puede en todas—, y el **cupo de planes** con su leyenda para la UI (cuántos quedan, el límite alcanzado, y que no quede en negativo si el superadmin baja el límite por debajo de lo ya cargado). Cubren además cómo se muestra un plan en la lista (qué versión ven los alumnos, si tiene cambios sin publicar) y el alta de un plan y de una universidad — el primer paso de una carga en una facultad ajena, que es el Gate C. Son las reglas que decide la **interfaz**: la que manda de verdad es la base, con sus políticas de RLS, y si las dos discrepan gana la base (RNF-12).
- **`Pasos` (12):** en qué parte de la carga está el plan (los tres pasos del editor). Lo importante que fijan estos tests es lo que el paso 2 **no** hace: no inventa un porcentaje de avance sobre las correlativas, porque no existe un denominador — las materias de primer año no tienen previas y están bien así. Y que un plan recién creado **no** muestra errores en el paso 3: el validador dice "no tiene ninguna materia", que es cierto y es ruido (no está roto, no empezaste).
...
flowchart LR
    ...
    L --> U["unit + integridad · vitest · 304"]
...
**El mismo gate corre en las ramas** (`.github/workflows/ci.yml`): en cualquier rama que no sea `main` y en los *pull requests*, con los mismos pasos pero **sin desplegar**. Antes, el gate solo existía en el push a `main`, así que trabajar en una rama era trabajar sin red — y como cada push a `main` publica en producción, el trabajo con idas y vueltas (un rediseño, por ejemplo) necesita una rama que igual esté vigilada.

## 6.6 Verificaciones que no viven en CI

Tres cosas importantes no se pueden automatizar en el pipeline, y conviene tenerlas escritas antes que suponerlas cubiertas:

- **Las políticas de acceso (RLS).** `supabase/003-verificar-permisos.sql` es el equivalente a "tests de seguridad con tres sesiones", pero sin OAuth: se hace pasar por cada usuario con `set_config('request.jwt.claims', …)` sobre cuentas reales y corre **17 chequeos** dentro de un *savepoint* que se deshace antes de devolver los resultados. El más importante es el aburrido —un administrador pasando a mano el id de otra universidad— y el que sostiene la promesa del producto: **ni el superadministrador ve el progreso de un alumno**. Corre en el editor SQL de Supabase, no en GitHub Actions: es el precio de probar RLS de verdad. Devuelve una **tabla** con el resultado de cada chequeo, no mensajes de consola — una verificación que no se ve no verifica nada.
- **Lo que necesita una sesión real.** El flujo de OAuth y todo el I/O que depende de una sesión iniciada (la lista de planes con datos reales, el sync entre dos dispositivos) se verifican a mano. Es un límite conocido: el bug de producción del 8-ago se escapó exactamente por ahí, y la respuesta fue mover a tests todo lo que sí se puede probar sin sesión — qué consulta se arma — inyectando el cliente por constructor.
- **Los cambios que tienen que verse igual.** Para un refactor puramente visual (la paleta a tokens, ADR-16: 141 reemplazos que **no debían cambiar un píxel**) se montó un arnés temporal de capturas con Playwright: cinco pantallas antes, las mismas cinco después. Cuatro salieron idénticas y el arnés **encontró un error real** —un token escrito y nunca definido, que dejó las aristas del esqueleto sin color, 1043 píxeles de diferencia—. Se borra cuando termina el refactor: es un instrumento de medición, no una suite.

**El tiempo, que es la fuente de flakiness más común.** Un test que falla a veces enseña a ignorar los rojos, que es peor que no tenerlo — así que cuando uno aparece, la pregunta no es "¿le subimos el timeout?" sino **contra qué está corriendo una carrera**. El caso que dio el proyecto: el e2e del interruptor de año fallaba una de cada tres corridas de la suite completa y pasaba siempre aislado. Esa firma —falla en grupo, pasa solo— es la de una carrera, no la de una rotura. La causa era que el aviso con acción **se auto-descarta a los 6 segundos**, y entre el clic que lo crea y el clic en "Deshacer" el test hace tres esperas que, con la suite en paralelo, se comen ese presupuesto. Se resolvió **congelando el reloj del navegador** (`page.clock`) en vez de tocar el producto: los `setTimeout` de la aplicación dejan de avanzar solos y el test mide lo que dice medir, no cuánto estaba cargada la máquina. La regla general: **un test nunca debe depender de que algo ocurra antes de que un temporizador de la aplicación lo borre.**
